...
std::string dataformat = "UNFILTERED";
...
int main()
{
  ...
  // Output root file
  const char *HFileNameCStr = HfileName.c_str();
  TFile* of = new TFile(HFileNameCStr, "RECREATE");
  ...
  int p = 0;
  p = countRootFilesInFolder(SfolderPath.c_str());
  DIR *directory = opendir(SfolderPath.c_str());
  if (directory == nullptr)
  {
    std::cerr << "Failed to open timesorted directory." << std::endl;
    return 1;
  }

  // Loop through all timesorted files
  for (int i = 0; i < p; i++)
  {
    std::string TfileName;
    if (i > 0) baseFileName = baseFileName + "_" + i + "_timesorted.root";
    else TfileName = baseFileName + "_timesorted.root";

    const char* TfileNameCStr = TfileName.c_str();
    
    // Open timesorted tree file
    TFile *file = TFile::Open(TfileNameCStr);
